...
// Formulaire complet : réservé aux admins et aux étudiants connectés (voir
// AddCompanyForm.vue, qui choisit ce formulaire ou ConfidentialCompanyForm.vue
// selon l'état de connexion, ou selon le format de la proposition en mode
// révision). Nom, adresse, site web, contacts et mission, comme avant le
...
// Un admin (ajout direct ou révision) est de confiance, l'étape d'attribution
// ne le concerne pas. Un étudiant connecté (seul autre cas où ce formulaire
// est affiché, voir AddCompanyForm.vue) est toujours attribué : cette étape
// se contente d'afficher son email et son choix de visibilité, elle ne
// bloque jamais la suite (il est déjà connecté par construction).
const showAttributionStep = computed(() => !isAdmin.value && !isReviewMode.value);
...
// par son numéro, car l'étape d'attribution n'existe que pour certains
// utilisateurs et décalerait sinon toutes les autres.
const stepKeys = computed(() => [
  ...(showAttributionStep.value ? ['student'] : []),
...
// formulaire depuis la dernière étape. L'étape d'attribution ("student") ne
// bloque jamais : ce formulaire n'est affiché qu'à un admin ou un étudiant
...
// envoie la proposition en attente de validation pour un étudiant connecté.
const submitForm = async () => {
  if (isLoading.value) return;

  // Filet de sécurité : la session étudiante a pu être fermée (déconnexion,
  // autre onglet) depuis l'ouverture du formulaire.
  if (showAttributionStep.value && !studentEmail.value) {
    currentStep.value = 1;
  ...
  try {
    ...
    if (props.pendingCompany) {
      ...
      await submitPending(company, { visible: submitterVisible.value, contacts: validContacts });
      submissionDone.value = true;
    }
  } catch (e) {
    console.error("Erreur lors de l'ajout de l'entreprise : ", e);
    logError(e, 'fullCompanyForm:submit');
  } finally {
...
const handleReject = async () => {
  ...
  try {
    ...
    logError(e, 'fullCompanyForm:reject');
  } finally {
    isLoading.value = false;
  }
};
...
<template>
  <div class="form-map-wrapper">
    ...
    <form v-else class="form-container" @submit.prevent="handleSubmit">
      ...
      <!-- Étape d'attribution : affiche l'email de l'étudiant connecté et son
